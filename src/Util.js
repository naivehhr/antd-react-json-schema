 ...
 */
export const processRely = (schema = {}, formData = {}, originalSchema = {}) => {
    let _schema = _.cloneDeep(schema)
    // console.log(_schema)
 ...
 * @param {*} pathStr 
 */
export const formatAmountValue = (schema = {}, formData = {}, pathStr = '', type = 'string') => {
    Object.keys(schema.properties).forEach((k, i) => {
        if (schema.properties[k].type == 'object') {
            ...
            formatAmountValue(schema.properties[k].items, formData, k, schema.properties[k].type)
...
export const _toLocaleString = (num, maximumFractionDigits = 0) => {
    ...
    if (num.match(/\./g) && (num.match(/\./g).length == 1)) {
        ...
        _num = parseFloat(num + '').toString().substring(0, num.lastIndexOf('.') + 1)
    }
    return parseFloat(_num).toLocaleString('en-US', { maximumFractionDigits: maximumFractionDigits })
}
...
const digitalAmountUnFormat = (num, schema) => {
    ...
    let _num = num
    if (num.match(/\./g) && (num.match(/\./g).length == 1)) {
        _num = (num + '').substring(0, num.indexOf('.') + 1 + (schema.precision || 2))
    } else if (num.match(/\./g) && (num.match(/\./g).length > 1)) {
        _num = (num + '').substring(0, num.lastIndexOf('.'))
    }
    let arr = _num.split('')
    _num = arr.length > 0 && arr.reduce((p, c) => {
        if (c == '.' || !isNaN(c)) {
            return p + c
        } else {
            return p
...
export const phoneNumberFormat = (str) => {
    if (isNaN(str) || str.length > 11) {
        return str.slice(0, str.length - 1).replace(/(\d{3})(?=\d)/, "$1 ").replace(/(\d{4})(?=\d)/g, "$1 ")
    }
    return !str ? str : (str + '').replace(/(\d{3})(?=\d)/, "$1 ").replace(/(\d{4})(?=\d)/g, "$1 ")
}
